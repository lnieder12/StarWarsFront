...
export class HpLeftFilter implements ClrDatagridNumericFilterInterface<Round> {
  accepts(item: Round, low: number, high: number): boolean {
    if (low == null)
      return item.hpLeft <= high;
    if (high == null)
      return low <= item.hpLeft;
    return low <= item.hpLeft && item.hpLeft <= high;
  }
